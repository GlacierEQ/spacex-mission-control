...
import math
from alpha.telemetry_aggregator import TelemetryAggregator, TelemetryPoint, MetricDefinition, MetricType, SourceType
from omega.cross_domain_fusion import CrossDomainFusionSystem, DomainState


def test_aggregator_ingest():
    agg = TelemetryAggregator()
    agg.define_metric(MetricDefinition(name="altitude", metric_type=MetricType.GAUGE, unit="m"))
    agg.register_source("vehicle", SourceType.VEHICLE)
    point = TelemetryPoint(source="vehicle", metric="altitude", value=1000, timestamp=1.0)
    assert agg.ingest(point)

def test_cross_domain_fusion():
    system = CrossDomainFusionSystem()
    system.ingest_domain_state(DomainState(
        domain="propulsion",
